...
async function newAnswer(index: number) {
  ...
  _.set(state.template, `questions[${index}].answers`, [
    ...question.answers,
    answer,
  ]);
  await save();
...
function switchValid(
  ...
  _.set(
    state.template,
    `questions[${questionIndex}].answers[${answerIndex}].valid`,
    !currentValue
  );
  return save();
}
...
<template>
  <div class="p-10" v-if="state.template">
    <div
      class="mx-auto md:w-2/3 bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700"
    >
      <div class="p-6">
        <img
          class="mx-auto mb-3 w-24 h-24 shadow-lg"
          :src="state.template.cover_url"
          alt="Bonnie image"
        />
        <a href="#" class="">
          <h5
            ...
            class="text-center mb-2 text-2xl font-semibold tracking-tight text-gray-900 dark:text-white"
          >
            {{ state.template.name }}
          </h5>
        </a>
